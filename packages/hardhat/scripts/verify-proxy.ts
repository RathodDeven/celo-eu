...
import { verifyUUPSProxy, parseDeploymentArgs, loadDeploymentInfo } from "./utils/deploymentUtils"
...
async function main() {
  ...
  if (!contractName) {
    ...
    console.log("   1. CONTRACT_NAME=NexusExplorerBadge pnpm hardhat run scripts/verify-proxy.ts --network alfajores")
    console.log("   2. pnpm hardhat run scripts/verify-proxy.ts --network alfajores NexusExplorerBadge")
    process.exit(1)
  }
  ...
  try {
    ...
    if (!deploymentInfo) {
      console.error(`❌ No deployment info found for ${contractName} on ${hre.network.name}`)
    ...
    if (verified) {
      ...
      const baseUrl = hre.network.name === 'celo' ? 'celoscan.io' : 'alfajores.celoscan.io'
      console.log(`🔗 Proxy: https://${baseUrl}/address/${deploymentInfo.proxyAddress}`)
      console.log(`🔗 Implementation: https://${baseUrl}/address/${deploymentInfo.implementationAddress}`)
      console.log("")
      console.log("💡 The proxy should now show 'Read as Proxy' and 'Write as Proxy' tabs")
    } else {
